  ...
  require(tidyverse)
  cov <- read.csv("./Telemetry Data/Covariates_by_cell.csv") %>%
    mutate(
  ...
  predict_use <- function(int, beta, cov) {
    predict_lam <- c()
    predict_prob <- c()
    for(i in 1:nrow(cov)) {
      predict_lam[i] <- exp(int + beta*cov[i])
  ...
  predict_use2 <- function(int, beta1, beta2, cov1, cov2) {
    predict_lam <- c()
    predict_prob <- c() 
    for(i in 1:nrow(cov)) {
      predict_lam[i] <- exp(int + beta1*cov1[i] + beta2*cov2[i])
      #predict_lam[i] <- exp(mu_a0_elk + b_elev_elk*cov$zDEM[i] + b_road_elk*cov$zRoads[i])
      predict_prob[i] <- 1-exp(-predict_lam[i])
    }
    predict_prob <- as.data.frame(predict_prob)
    return(predict_prob)
  ...
  #' Try to combine info from two covariates using predict_use2
  #' Combo model
  #'   coug_combo_el_rd_predict_elev <- predict_use(mu_a0_coug, b_elev_coug, elev)  
  elk_combo_el_rd_predict_road_elev <- predict_use2(mu_a0_elk, b_road_elk, b_elev_elk, road, elev)
  coug_combo_el_rd_predict_road_elev <- predict_use2(mu_a0_coug, b_road_coug, b_elev_coug, road, elev)
  #' Telemetry Only Model
  elk_telem_el_rd_predict_road_elev <- predict_use2(0, b_road_elk, b_elev_elk, road, elev)
  coug_telem_el_rd_predict_road_elev <- predict_use2(0, b_road_coug, b_elev_coug, road, elev)
  #' Camera Only Model
  elk_cam_el_rd_predict_road_elev <- predict_use2(mu_a0_elk_cam, b_road_elk, b_elev_elk, road, elev)
  coug_cam_el_rd_predict_road_elev <- predict_use2(mu_a0_coug_cam, b_road_coug, b_elev_coug, road, elev)
  
  #'  Now plot these results!
  #'  Load spatial data
  grid <- raster("./NE_grid_4k.img")
  crs(grid)
  crs(NE_sa)
  NE_sa <- st_read(getwd(), layer = "NE_SA") %>%
    st_transform(., crs = st_crs(grid))
  NE_box <- st_read(getwd(), layer = "NE_covariate_area_2855")
  crs(NE_box)
  ...
  #' try for combining two covariates
  elk_combo_df_roads_elev <- rasterize_predictions(cells, elk_combo_el_rd_predict_road_elev, coords, grid)
  coug_combo_df_roads_elev <- rasterize_predictions(cells, coug_combo_el_rd_predict_road_elev, coords, grid)
  elk_telem_df_roads_elev <- rasterize_predictions(cells, elk_telem_el_rd_predict_road_elev, coords, grid)
  coug_telem_df_roads_elev <- rasterize_predictions(cells, coug_telem_el_rd_predict_road_elev, coords, grid)
  elk_cam_df_roads_elev <- rasterize_predictions(cells, elk_cam_el_rd_predict_road_elev, coords, grid)
  coug_cam_df_roads_elev <- rasterize_predictions(cells, coug_cam_el_rd_predict_road_elev, coords, grid)
  ...
  pdf("./ResultsMaps/Map_Elk_Probability_Use_Elevation.pdf")
  plot_elk_use(NE_sa, elk_combo_df_elev, elk_telem_df_elev, elk_cam_df_elev)
  dev.off()
  pdf("./ResultsMaps/Map_Elk_Probability_Use_RoadDensity.pdf")
  plot_elk_use(NE_sa, elk_combo_df_road, elk_telem_df_road, elk_cam_df_road)
  dev.off()
  
  pdf("./ResultsMaps/Map_Elk_Probability_Use_RoadDensity_Elevation.pdf")
  plot_elk_use(NE_sa, elk_combo_df_roads_elev, elk_telem_df_roads_elev, elk_cam_df_roads_elev)
  dev.off()
  ...
  pdf("./ResultsMaps/Map_Cougar_Probability_Use_RoadDensity_Elevation.pdf")
  plot_coug_use(NE_sa, coug_combo_df_roads_elev, coug_telem_df_roads_elev, coug_cam_df_roads_elev)
  dev.off()
  ...
  tst <- ggplot() +
    geom_sf(data = NE_sa) +
    geom_raster(data = coug_combo_df_road, aes(x = x, y = y, fill = predict_prob)) +
    geom_sf(data = NE_sa, fill = NA, color = "black") +
    scale_fill_distiller(palette = "OrRd", trans = "reverse") +
